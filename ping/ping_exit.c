...
<opt>       : <opt>* (options can be combined)
              ...
              NOTES:  - n and N are mutually exclusive
                      - m can be combined with n or N and will then output <x>/<map> where <x> is the output generated by
                       'n' or 'N' respectively, and <map> the map generated by 'm'
                      ...
                      - ping may have a documented hard coded default limit for the map size (for example 100) but this
                        size should be overridable (>100) by the m(<n>) option passing the responsibility for memory use
                        to the user
                      - the N option may output 3 fields success>/<failures>/<total> making counter wraps detectable
                        (at least in relevant cases) but this should not be necessary as with the default interval of 1
                        second it would take a minimum of over 136 years before a 32-bit unsigned int would wrap. Even in
                        flood mode (which is not expected to be used continuously) with an assumed 2ms average (only
                        successful) round-trip rate it would take over 2.7 years. (Optionally) Using 64-bit counters
                        would make the whole topic obsolete.
                      - the N option may also be turned into a ping flag (like -C) as I think that this type of output
                        could be interesting for scripting also without the use of an exit condition
 */
#include "ping.h"
